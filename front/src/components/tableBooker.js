...
const TableBooker = (user_id) => {
  const dispatch = useDispatch();
  const dates = useSelector(selectDates);
  const users = useSelector(selectBookers);
  const actualUser =
    users && users.find((user) => user.user_ID === user_id.user_id);
  ...
  return (
    <>
      <h2>Date de {actualUser != null && actualUser.user_name}</h2>
        ...
        <tbody>
          {dates &&
            dates.map((date, index) => {
              if (date.user.user_ID === user_id.user_id) {
                return (
                  <tr key={index}>
                    <td>{date.date}</td>
