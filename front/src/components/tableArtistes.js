...
const TableArtistes = (artiste_id) => {
  const dispatch = useDispatch();
  const dates = useSelector(selectDates);
  const artistes = useSelector(selectArtistes);
  const actualArtiste =
    artistes &&
    artistes.find((artiste) => artiste.artiste_ID === artiste_id.artiste_id);
  ...
  return (
    <>
      <h2>Date de {actualArtiste != null && actualArtiste.artiste_name}</h2>
        ...
        <tbody>
          {dates &&
            dates.map((date, index) => {
              if (date.artiste.artiste_ID === artiste_id.artiste_id) {
                return (
                  <tr key={index}>
                    <td>{date.date}</td>
