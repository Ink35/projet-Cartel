...
const TablePlaces = (place_id) => {
  const dispatch = useDispatch();
  const dates = useSelector(selectDates);
  const places = useSelector(selectPlaces);
  const actualPlace =
    places && places.find((place) => place.place_ID === place_id.place_id);
  ...
  return (
    <>
      <h2>Date de {actualPlace != null && actualPlace.place_name}</h2>
        ...
        <tbody>
          {dates &&
            dates.map((date, index) => {
              if (date.place.place_ID === place_id.place_id) {
                return (
                  <tr key={index}>
                    <td>{date.date}</td>
